  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const plugins = await Promise.all(listThirdParty(dir).map(async (p) => {
        const type = String(p.spec).startsWith('github:') ? 'github' : 'npm'
        let installed = null
        let latest = null
        let error = null
        if (type === 'github') {
          const repo = githubRepoOf(p.spec)
          if (repo === null) {
            error = '无法解析仓库地址'
          } else {
        ...
        const hasUpdate = installed !== null && latest !== null
          && (type === 'github' ? installed !== latest : isNewer(latest, installed))
        return { name: p.name, spec: p.spec, enabled: p.enabled, type, installed, latest, hasUpdate, error }
      }))
