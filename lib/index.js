 ...
 * route uses.
 *
 * `add` also passes `--config.minimumReleaseAge=0`: pnpm 11 otherwise
 * SILENTLY keeps the old version and exits 0 when the requested release is
 * younger than its fresh-release safety window, so an update looks like a
 * no-op ("Already up to date"). The user already clicked update, so the
 * wait is waived for this one command.
 */
function runPluginCommand(bootstrap, profile, action, target, onDone) {
  ...
  const args = ['--expose-internals', b.dshBootstrapPath, 'plugin', '--profile', profile, action]
  if (action === 'add') args.push('--config.minimumReleaseAge=0')
  args.push(target)
  execFile(
    b.appExecutable,
    args,
